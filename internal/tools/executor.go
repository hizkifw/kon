...
// turn ending.
func (e *Executor) Execute(ctx context.Context, name string, arguments json.RawMessage) (Result, bool) {
	tool, ok := e.registry.Lookup(name)
	if !ok {
		return Result{Content: fmt.Sprintf("error: unknown tool %q", name)}, true
	}
	result, err := tool.Run(ctx, Env{cwd: e.cwd, vision: e.vision}, arguments)
	if err != nil {
